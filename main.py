...
def get_new_label_by_LLM_result_rate(label_num=6):
    import pandas as pd
...
def convert_LLM_answer_to_label(MODEL_NAME):
    ...
    output_file_name = LLM_RESULT_DIR / ("VUA20_LLM_result_" + MODEL_NAME + ".csv")
...
def LLM_result_label_combine(MODEL_NAME, LABEL_NUM):
    ...
    # File paths
    main_file = LLM_RESULT_DIR / ("VUA20_LLM_result_" + MODEL_NAME + ".csv")
    ...
    # Extract the label column from result_labeled.csv
    ...
    output_file = LLM_RESULT_DIR / ("VUA20_LLM_result_" + MODEL_NAME + "_labeled.csv")
...
def cal_accuracy_by_bin(MODEL_NAME, num_classes=3):
    ...
    csv_file = LLM_RESULT_DIR / ("VUA20_LLM_result_" + MODEL_NAME + "_labeled.csv")

    # Read the CSV file
    df = pd.read_csv(csv_file)

    # Ensure that the CSV file has at least 9 columns.
    # The 8th column is the similarity level, and the 9th column is the predicted label.
    if df.shape[1] < 9:
        raise ValueError("The CSV file has fewer than 9 columns.")

    # Extract similarity levels from the 8th column and predicted labels from the 9th column
    similarity = df.iloc[:, 7]  # Similarity column
    labels = df.iloc[:, 8]      # Predicted label column

    # Select valid similarity levels according to the number of classes
    if num_classes == 3:
        valid_levels = [0, 1, 2]
    elif num_classes == 6:
